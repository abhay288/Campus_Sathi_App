    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          icon: <Maximize2 className="w-6 h-6 text-violet-600" />,
      ...
      items: [
        {
          ...
          icon: <FileCheck2 className="w-6 h-6 text-rose-600" />,
    ...
    <>
      ...
      <div className="py-12 md:py-20 bg-slate-50 border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
          ...
          <div className="space-y-12">
            {tools.map((section, sIdx) => (
              <div key={sIdx} className="space-y-6">
                <h2 className="text-xl font-extrabold text-slate-900 tracking-tight border-b border-slate-200 pb-3 flex items-center space-x-2.5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {section.items.map((tool, tIdx) => (
                    <div 
                      key={tIdx} 
                      className="p-7 rounded-2xl bg-white border border-slate-200 shadow-sm card-hover flex flex-col justify-between space-y-4"
                    >
          ...
          <div className="p-8 rounded-2xl bg-white border border-teal-200/80 shadow-sm space-y-3">
            <div className="flex items-center space-x-3">
              <div className="p-2.5 rounded-xl bg-teal-50 border border-teal-200 text-teal-600">
                <ShieldCheck className="w-6 h-6" />
              </div>
              <div>
                <h3 className="text-lg font-bold text-slate-900">Smart Scanner: Local Document Privacy</h3>
                <p className="text-xs text-slate-500 font-medium">Your documents stay on your device.</p>
              </div>
            </div>
            <p className="text-slate-600 text-sm leading-relaxed">
          ...
          <div className="p-6 rounded-2xl bg-amber-50/80 border border-amber-200 flex items-start space-x-3.5 text-xs text-slate-700">
            <AlertTriangle className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
            <div className="space-y-1">
              <span className="font-bold text-amber-900">ATS Analysis Informational Notice:</span>
